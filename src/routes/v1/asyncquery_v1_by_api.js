  ...
  setRoutes(app) {
    app
      ...
      .post(swaggerValidation.validate, async (req, res, next) => {
        if (!global.queryQueueByAPI) {
          global.queryQueueByAPI = getQueryQueue("bte_query_queue_by_api");
          global.queryQueueByAPI.process(
        ...
        await asyncquery(req, res, next, queueData, global.queryQueueByAPI);
      })
