...
async function jobToBeDone(queryGraph, caching){
...
queryQueue.process(async (job) => {
    return jobToBeDone(job.data.queryGraph, job.data.caching);
});
    ...
    setRoutes(app) {
        app.post('/v1/async_query', swaggerValidation.validate, async (req, res, next) => {
            try {
                ...
                let job = await queryQueue.add({queryGraph: req.body.message.query_graph, caching: req.query.caching});
